...
public class MyImage extends View{
    ...
    MyImage(Context context){//문맥 설정 (현재 위치 : this)
        ...
        picture= BitmapFactory.decodeResource(getResources(),R.drawable.doo); //sizuku라는 이미지 파일을 bitmap파일로 변환하여 반환하는 역활 입니다!
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float cx=getWidth()/2.0f; //view의 정 중앙 좌표!
        float cy=getWidth()/2.0f;
        float x=(getWidth()-picture.getWidth())/2.0f; //draw bitmap이 float형 이여서 float형이 좋음!
        float y=(getHeight()-picture.getHeight())/2.0f;  //정 중앙에 위치 하도록 함!

        switch (choose){
            case MainActivity.ROTATE:
                canvas.rotate(15,cx,cy);//회전
                break;
            case MainActivity.TRANSLATE:
                canvas.translate(-150,200); //이동
                break;
            case MainActivity.SCALE:
                canvas.scale(1.5f,1.0f,cx,cy); //크기 변경 0.2배
                break;
